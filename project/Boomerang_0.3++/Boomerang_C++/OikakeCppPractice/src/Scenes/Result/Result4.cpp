...
Result4::Result4(WorldPtr & world)
	: isEnd(false)
	, world(world)
...
void Result4::Update(float deltaTime)
{
...
bool Result4::IsEnd() const
{
	return isEnd;
}

Scene Result4::Next() const
{
	return nextScene;
}

void Result4::Finalize()
{
	world->Finalize();
	renderer.Clear();
}

void Result4::HandleMessage(EventMessage message, void * param)
{
