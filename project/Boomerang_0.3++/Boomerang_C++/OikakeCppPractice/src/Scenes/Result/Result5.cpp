...
Result5::Result5(WorldPtr & world)
	: isEnd(false)
	, world(world)
...
void Result5::Update(float deltaTime)
{
...
bool Result5::IsEnd() const
{
	return isEnd;
}

Scene Result5::Next() const
{
	return nextScene;
}

void Result5::Finalize()
{
	world->Finalize();
	renderer.Clear();
}

void Result5::HandleMessage(EventMessage message, void * param)
{
