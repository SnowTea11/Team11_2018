...
void Stage2::Initialize()
{
	...
	world->AddActor_Back(ActorGroup::Block, std::make_shared<Block>(world.get(), Vector2(312,408)));
	...
	world->AddActor_Back(ActorGroup::Enemy, std::make_shared<Enemy>(world.get(), Vector2(600,120)));
	world->AddActor_Back(ActorGroup::Enemy, std::make_shared<Enemy>(world.get(), Vector2(168,264)));
	world->AddActor_Back(ActorGroup::Enemy, std::make_shared<Enemy>(world.get(), Vector2(1032,264)));
...
bool Stage2::IsEnd() const
{
	return isEnd;
}

Scene Stage2::Next() const
{
	return Scene::Result2;//リザルト画面へ
}
...
void Stage2::HandleMessage(EventMessage message, void * param)
{
