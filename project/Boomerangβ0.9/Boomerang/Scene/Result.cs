...
namespace Boomerang
{
    class Result : IScene
    {
        ...
        public void Update()
        {
            ...
            if (input.IsKeyDown(Keys.Left) || input.IsPadDown(Buttons.DPadLeft))
            {
                cursorPosition = 120;
            }
            if (input.IsKeyDown(Keys.Right) || input.IsPadDown(Buttons.DPadRight))
            {
                cursorPosition = 650;
            }

            if (input.IsKeyDown(Keys.Space) || input.IsPadDown(Buttons.A))
            {
                SceneCange();
                this.isEnd = true;
